...
export function initTerminal(rootSelector) {
	...
	function resolvePath(pathStr) {
		const segments = pathStr.startsWith("/") || pathStr.startsWith("~") ? [] : [...cwd];
	...
	function ls(arg) {
		...
		if (Array.isArray(node)) return [arg];
		if (!isDir(node)) return [`ls: ${arg}: no such file or directory`];
		const names = Object.keys(node).map((name) => (isDir(node[name]) ? name + "/" : name));
		return [names.join("   ")];
	}

	function cd(arg) {
		const segments = resolvePath(arg || "");
		const node = nodeAt(segments);
		if (isDir(node)) { cwd = segments; return []; }
		if (Array.isArray(node)) return [`cd: not a directory: ${arg}`];
		return [`cd: no such directory: ${arg}`];
	}

	function cat(arg) {
		if (!arg) return ["usage: cat <file>"];
		const target = nodeAt(resolvePath(arg));
